...
function populateTable() {
    ...
    $.getJSON( '/api/getGzip/', function( data ) {
...
function showProxyInfo(event) {
    ...
    $.each(proxyListData, function(){
    	if(this.configid==thisProxyHost)
    		{
    		//Populate Info Box
    	    $('#proxyID').text(this.configid);
    	    $('#updateProxy fieldset label#updateProxyURL').text(this.proxyurl);
    	    $('#updateProxy fieldset input#updateProxyTargetURL').val(this.targeturl);
    		}
    })
    // Get Index of object based on id value
   // var arrayPosition = proxyListData.map(function(arrayItem) { var arrayPosition = proxyListData.map(function(arrayItem) {$('#proxyID').text(arrayItem.configid); $('#updateProxy fieldset label#updateProxyURL').text(arrayItem.proxyurl);
    //$('#updateProxy fieldset input#updateProxyTargetURL').val(arrayItem.targeturl); return arrayItem.configid; }).indexOf(thisProxyHost);
 //return arrayItem.configid; }).indexOf(thisProxyHost);

    // Get our User Object
    //var thisProxyObject = proxyListData[thisProxyHost-1];
...
function deleteProxy(event) {
    ...
    if (confirmation === true) {
        ...
        $.ajax({
                 type : 'DELETE',
                 data: '',
                 url : '/api/createGzip/'+$(this).attr('rel'),
                 dataType : 'JSON'
