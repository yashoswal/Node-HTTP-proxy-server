...
function populateTable() {
    ...
    $.getJSON( '/api/cacheproxy/', function( data ) {
...
function updateProxy(event) {
    ...
    if(errorCount === 0) {
        ...
        $.ajax({
            type: 'PUT',
            data: newProxy,
            url: '/api/cacheproxy/'+id, // + $('#proxyID').text(),
            dataType: 'JSON'
        }).done(function( response ) {
...
function deleteProxy(event) {
    ...
    if (confirmation === true) {
        ...
        $.ajax({
                 type : 'DELETE',
                 data: '',
                 url : 'http://localhost:8006/cacheproxyserver/stop/'+$(this).attr('rel'),
                 dataType : 'JSON'
        ...
        $.ajax({
            type: 'DELETE',
            data: newProxy,
            url: '/api/cacheproxy/'+$(this).attr('rel'),
            dataType: 'JSON'
        }).done(function( response ) {
...
function stopProxyServer(){
    ...
    if(errorCount == 0)
    {
        $.ajax({
                 type : 'DELETE',
                 data: '',
                 url : 'http://localhost:8006/cacheproxyserver/stop/'+configid,
                 dataType : 'JSON'
...
function startProxyServer(){
    ...
    if(errorCount == 0)
    {
        ...
        $.ajax({
             type : 'POST',
             data : data,
             url : 'http://localhost:8006/cacheproxyserver/start',
             dataType : 'JSON'
